...
class BinaryTree:
    ...
    def _preorder(self, p):  # p is the root of subtree
        if p is not None:
            return
    ...
    def height(self):
        return self._height(self.root)  # Returning the result of the recursive function

    def _height(self, p):
        if p is None:
            return 0

        hL = self._height(p.lchild)     # Finding the height recursively
