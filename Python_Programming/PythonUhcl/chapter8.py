...
def count2(strng):
    #strng.split(" ")
    joinedstrn = ""
    count_words_with_e = 0
    words = len(strng)
    for word in strng:
        if "E".lower() in word:
            count_words_with_e += 1
    joinedstrn= joinedstrn.join(strng)
    count = joinedstrn.count("e")
    print("The total words in the text: {w} \nThe total 'e's are: {es} \nthe total words that have an 'e': {ce}".format(w = words, es = count, ce = count_words_with_e ))


#counts("is", "Mississippi")
# reverse2("good")
reverse2("Python")
reverse('happy')
