...
db.users.findOneAndUpdate({ name: 'Max Jones' }, [
	{ $set: { name_array: { $split: ['$name', ' '] } } },
	{
		$set: {
			first_name: { $arrayElemAt: ['$name_array', 0] },
...
db.users.findOneAndUpdate({ name: 'The Sleeper' }, [
	{ $set: { name_array: { $split: ['$name', ' '] } } },
	{
		$set: {
			first_name: { $arrayElemAt: ['$name_array', 0] },
...
db.movies.findOneAndUpdate({ _id: 111 }, { $set: { 'genres.$[]': 'Action' } }, { returnNewDocument: true });
